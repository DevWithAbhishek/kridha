...
const LUCKNOW_LNG = 80.9462;

export function DealsSection() {
  const t = useTranslations("deals");

  const { data: deals, loading } = useFetch<Deal[]>(
    `/api/products/deals?lat=${LUCKNOW_LAT}&lng=${LUCKNOW_LNG}`,
    [],
  );
    ...
    <section className="bg-[var(--color-surface)] dark:bg-gray-900 py-section-y">
      <div className="max-w-page mx-auto px-page-x md:px-page-x-md">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            href="/products?dealActive=true"
            className="text-kridha-primary hover:underline"
          >
            {t("see_all")}
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory lg:grid lg:grid-cols-3 lg:gap-6 lg:overflow-visible">
            {deals?.map((deal) => (
              <div
                key={deal.id}
                className="bg-[var(--color-surface)] dark:bg-surface-dark rounded-card shadow-card border border-[var(--color-border)] p-card min-w-[280px] snap-center lg:min-w-0"
              >
                <div className="relative mb-4">
                  ...
                  <div className="aspect-product rounded-lg bg-kridha-secondary dark:bg-gray-700 flex items-center justify-center">
                    <Image
                      src={deal.imageUrl || "/images/placeholder.svg"}
                      alt={deal.nameHi}
                      width={200}
                      height={200}
                ...
                <p className="text-label-sm text-[var(--color-text-muted)] mb-2">
                  {deal.seller.storeName}
                </p>
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-kridha-primary font-bold">
